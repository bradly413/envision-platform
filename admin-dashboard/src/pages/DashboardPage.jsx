...
export default function DashboardPage() {
  ...
  const clientsByStage = overview?.clientsByStage?.length
    ...
    : Object.entries(allClients.reduce((acc, c) => { acc[c.stage] = (acc[c.stage] || 0) + 1; return acc; }, {}))
  ...
  const openTasksList = tasksData.filter(t => t.status !== 'done').slice(0, 5);
  ...
  return (
    <div style={{ padding: 28, fontFamily: 'Inter, sans-serif', maxWidth: 1300, height: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setShowAddClient(true)} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #E5E7EB', background: '#fff', fontSize: 13, fontWeight: 600, color: '#374151', cursor: 'pointer' }}>
            + Add client
          </button>
          <button onClick={() => navigate('/portals')} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#111827', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer' }}>
            + Create portal
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 14 }}>
        {[
          { label: 'Active clients', value: activeClients, color: '#10B981', sub: `${allClients.length} total`, to: '/pipeline' },
          { label: 'Open tasks', value: openTasks, color: '#3B82F6', sub: `${tasksData.filter(t => t.status === 'done').length} done`, to: '/tasks' },
          { label: 'Live portals', value: livePortals, color: '#8B5CF6', sub: `${portalsData.length} total`, to: '/portals' },
          { label: 'Pipeline revenue', value: `$${revenue.toLocaleString()}`, color: '#F59E0B', sub: 'all stages', to: '/pipeline' },
        ].map(stat => (
          <div key={stat.label} onClick={() => navigate(stat.to)} style={{ ...CARD, cursor: 'pointer', transition: 'box-shadow .15s, transform .15s' }}
            ...
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: '#9CA3AF', marginBottom: 10 }}>{stat.label}</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: stat.color, lineHeight: 1, marginBottom: 6 }}>{stat.value}</div>
            <div style={{ fontSize: 11, color: '#C4C9D4', marginTop: 'auto' }}>{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>Client portals</div>
            <button onClick={() => navigate('/portals')} style={{ fontSize: 11, color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer' }}>View all →</button>
          </div>
          {recentPortals.length ? recentPortals.map(p => (
            <div key={p.id} onClick={() => navigate('/portals')} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid #F9FAFB', cursor: 'pointer' }}
              onMouseOver={e => e.currentTarget.style.opacity = '.7'}
              onMouseOut={e => e.currentTarget.style.opacity = '1'}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 30, height: 30, borderRadius: 8, background: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#6B7280', flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 20, background: STATUS_BG[p.status] || '#F3F4F6', color: STATUS_COLORS[p.status] || '#6B7280' }}>
                {p.status || 'draft'}
              </span>
            </div>
          )) : (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 0' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 12, color: '#9CA3AF' }}>No portals yet —{' '}
                  <span onClick={() => navigate('/portals')} style={{ color: '#111827', fontWeight: 600, cursor: 'pointer' }}>create one</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>Open tasks</div>
            <button onClick={() => navigate('/tasks')} style={{ fontSize: 11, color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer' }}>View all →</button>
          </div>
          {openTasksList.length ? openTasksList.map(t => (
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 0' }}>
              <div style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>All caught up — no open tasks</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>Pipeline by stage</div>
            <button onClick={() => navigate('/pipeline')} style={{ fontSize: 11, color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer' }}>View pipeline →</button>
          </div>
          {clientsByStage.length ? (
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(clientsByStage.length, 5)}, 1fr)`, gap: 8 }}>
              {clientsByStage.map(({ stage, count }) => (
                <div key={stage} onClick={() => navigate('/pipeline')} style={{ background: '#F9FAFB', borderRadius: 8, padding: '14px 12px', borderTop: `3px solid ${STAGE_COLORS[stage] || '#E5E7EB'}`, cursor: 'pointer', transition: 'box-shadow .15s' }}
                  onMouseOver={e => e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,.06)'}
                  onMouseOut={e => e.currentTarget.style.boxShadow = 'none'}>
                  <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.07em', color: '#9CA3AF', marginBottom: 6 }}>{stage}</div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 0' }}>
              <div style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>No clients yet —{' '}
                <span onClick={() => setShowAddClient(true)} style={{ color: '#111827', fontWeight: 600, cursor: 'pointer' }}>add one</span>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Internal notes */}
        <div style={CARD}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#111827', marginBottom: 14 }}>Internal notes</div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            <input value={note} onChange={e => setNote(e.target.value)} onKeyDown={e => e.key === 'Enter' && addNote()}
              placeholder="Add a note..."
              style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid #E5E7EB', fontSize: 13, outline: 'none', fontFamily: 'Inter, sans-serif' }} />
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            )) : <div style={{ fontSize: 12, color: '#9CA3AF' }}>No notes yet — add one above</div>}
          </div>
        </div>
      </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: 440, boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
            ...
            {[
              ...
              <div key={key} style={{ marginBottom: 13 }}>
                ...
                <input value={clientForm[key]} onChange={e => setClientForm(f => ({ ...f, [key]: e.target.value }))} placeholder={placeholder}
